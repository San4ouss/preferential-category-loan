...
@name('S92. Список всех кредитов. Расширение')
@tag('EXTENSION')
view VW_CRIT_S92_PR_CRED_EXT instead of VW_CRIT_S92_PR_CRED {
    type main is
	select cred(
				...
				, (select x(case when count(1) > 0 then '{***}' else '{...}' end) in ::[DEVAL_HIST] all where x.[PRODUCT]%id = cred%id) : C_DEVALUATION
				, (select x(case when count(1) > 0 then '{***}' else '{...}' end) in ::[DEVAL_RESERV] all where x.[PRODUCT]%id = cred%id) : C_DEVAL_RESERV
				, (select x(case when count(1) > 0 then '{***}' else '{...}' end) in ::[PRC_DEVAL_HIST] all where x.[PRODUCT]%id = cred%id) : C_PRC_DEVAL
				, (select x(case when count(1) > 0 then '{***}' else '{...}' end) in ::[DEV_PRC_PAR_HIST] all where x.[PRODUCT]%id = cred%id and x.[PARAM_TYPE] = 'PD') : C_PD
				, (select x(case when count(1) > 0 then '{***}' else '{...}' end) in ::[DEV_PRC_PAR_HIST] all where x.[PRODUCT]%id = cred%id and x.[PARAM_TYPE] = 'LGD') : C_LGD
				...
				, (select o(case when count(1) > 0 then '{***}' else '{...}' end) in ::[CRED_CARD_DOC] all where o.[FACT_OPER]%collection = cred.[LIST_PAY]) : C_CCARD_DOC
				, nvl(cred.[DEPART], cred.[CREATE_USER]->(true, [USER])[DEPART]) : C_DEPART
				, cred.[FILIAL] : C_FILIAL
				, cred%class : C_CLASS_ID
				, ::[PR_CRED].[SQL_LIB_CR_001].getPortNumDog(cred, cred.[HIGH_LEVEL_CR]) : C_NUM_DOG_3
				...
				, (select rlt(case when count(1) > 0 then '{***}' else '{...}' end) in ::[FL_RATE_RELATION] all where rlt.[PRODUCT]%id = cred%id) : C_FL_RULE
				...
				, (select regl(case when count(1) > 0 then '{***}' else '{...}' end) in ::[REGL_OPER] where regl.[PRODUCT]%id = cred%id) : C_REGL_OPER
				...
				, (select uid(uid.[UID]) in ::[UID] all where uid.[PRODUCT] = cred%id) : UID
				, (select x(x.[NAME]) in ::[S92_CR_CATEGORY] inner join (::[S92_CR_TO_CATEG] all : to_categ)
					on x%id = to_categ.[CATEGORY] all
					where cred%id = to_categ.[CREDIT] and ::[SYSTEM].[OP_DATE] >= to_categ.[START] 
					and (to_categ.[END] is null or to_categ.[END] >= ::[SYSTEM].[OP_DATE])
					) : C_CATEGORY_NAME
			)
	in	::[PR_CRED]
		left outer join (::[DEBT_COMISS_PRC] all : commiss) on commiss%collection = cred.[COMISS_ARR]
			and commiss.[DEBT] = ::[VID_DEBT]([CODE] = 'НЕУЧТЕН_ПРОЦЕНТЫ' and [TO_PRODUCT] = ::[ACC_PRODUCT]([CODE] = 'PR_CRED'))
	all;
